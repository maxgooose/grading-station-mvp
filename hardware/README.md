...
**Where we are (2026-06-10):** shell assembled (door pending), all 5 cameras in hand, top camera installed + adjusted, **stage locked** (centered on the cam axis, landscape, 7.0"×3.5" phones-only envelope, screen-up first). Working order from here: **trace stage → place edge cameras → buy + install lighting** — cameras before lighting because the camera optics are the rigid constraint and the LED strips are the flexible one (they compete for the same screen-height band around the device).
...
5. **Place the 4 edge cameras (IMX323)** around the stage. Gate: measure the stock lens's actual sharp-focus distance first — stock 61°H + ~30cm fixed focus does not cover a 6.4" edge from ~4.4" away, so expect a wider close-focus M12 lens swap or a geometry workaround (corner mounts / standoffs).
