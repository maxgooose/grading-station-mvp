...
- **2026-06-08 — Camera layout LOCKED (5 cameras).** Top ELP 4K (lens through the roof hole) for flat faces + scratches; **4× IMX323 (3.6 mm) as side edge cameras**, mid-panel ~4" up each wall, angled ~40–50° down, framing the 4 edges + corners. FOV math, mount table, corner coverage, and the lighting interaction (dark-field = top-only; new edge bright-field state) are in [`camera-placement.md`](camera-placement.md).
...
4. **Install bright-field + dark-field lighting** — plug-and-play, **switched by hand** between the BF and DF shots (no MOSFET/PSU for v1; lighting eyeball test already PASSED — see progress log). Buy list + flicker caveat: [`v1-parts-order.html`](v1-parts-order.html). Auto-switching (USB relay / MOSFET) is an optional later upgrade.
