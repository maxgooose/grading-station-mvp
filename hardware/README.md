...
| [`camera-placement.md`](camera-placement.md) | **5-camera layout** (top ELP 4K + 4 side IMX323 edge cams): FOV math, mount heights/angles, corner coverage, capture sequence, lighting interaction. |
...
- **2026-06-08 — Camera layout specced on paper (5 cameras) — mount scheme SUPERSEDED 2026-06-11.** Top ELP 4K (lens through the roof hole) for flat faces + scratches; 4× IMX323 (3.6 mm) as side edge cameras, mid-panel ~4" up each wall, angled ~40–50° down. FOV math, mount table, and the lighting interaction (dark-field = top-only; new edge bright-field state) are in [`camera-placement.md`](camera-placement.md). *Reconciliation note:* this spec and the 06-10/06-11 corner-mount work below were done in parallel sessions. The corner-mount plan (decided with the boards in hand) is the build plan; the ~73° HFOV math in `camera-placement.md` was independently confirmed (≈74°), but its mid-wall mount table is historical — see the banner in that file.
...
6. **Buy + install lighting** — buy decision **locked 2026-06-08**: plug-and-play, **switched by hand** between the BF and DF shots (no MOSFET/PSU for v1; auto-switching via USB relay / MOSFET is an optional later upgrade). Buy list + flicker caveat: [`v1-parts-order.html`](v1-parts-order.html). Install to fit the locked camera geometry: dark-field strips + diffused bright-field source placed so their bodies and specular bounce stay out of all 5 camera FOVs (eyeball test already PASSED — see progress log).
