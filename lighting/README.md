...
The station has **5 cameras** (1 overhead ELP 4K + 4 corner-mounted ELP-USBFHD06H-L36 edge cams, lens center ~0.75" up, aimed level — corner layout confirmed 2026-06-11; see the progress log in [`../hardware/README.md`](../hardware/README.md)). That adds two things to the lighting:
...
- **Placement order matters:** the corner cams sit **low (lens ~0.75")** — the same height band as the grazing strips. Strips are sized/placed **after** the cameras are mounted, so their bodies and specular bounce stay out of all 5 camera FOVs. (The earlier "cams high ~4" / strips low — no collision" claim came from the superseded mid-wall layout in [`../hardware/camera-placement.md`](../hardware/camera-placement.md).)
