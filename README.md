...
- **Lighting:** dual-capture plan agreed. **Blocking:** zero-cost eyeball test (~15 min) has not been run yet — gates all downstream work. Once the wooden-plank shell is built and the lighting is installed, this test runs inside the actual enclosure.
...
3. **Build the wooden-plank capture shell** — matte black wooden panels (5–6 thin 12" panels for walls/roof, one thick 24"×12" plank as the floor), light-tight, sitting on top of the utility box. Lighting goes in first; cameras only after the lighting is verified.
4. Run the eyeball test inside the wooden-plank shell once lighting is installed ([`lighting/README.md`](lighting/README.md)).
